...
import com.example.newapp.Models.ProfileModel;
import com.example.newapp.Models.QuestionModel;
import com.example.newapp.Models.RankModel;
...
public class DbQuery {
               ...
               .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                   ...
                   public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                       Map<String, QueryDocumentSnapshot> docList = new ArrayMap<>();
                           ...
                           {
                               String catID = document.getString("CAT_ID");
                               String catName = document.getString("NAME");
                               int noOfTest = document.getLong("NO_OF_TESTS").intValue();
   ...
   public static void loadBookmarks(MyCompleteListener completeListener)
   {
       ...
       for(int i=0; i< g_bmIdList.size();i++)
       {
           ...
           g_firestore.collection("Questionsgoo").document(docID).get()
                   .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                       @Override
                       public void onSuccess(DocumentSnapshot documentSnapshot) {
                   ...
                   public void onSuccess(DocumentSnapshot documentSnapshot) {
                       if (documentSnapshot.exists()) {
                           ...
                           for (int i=1;i <= noOfTests; i++){
                               String testNo= "TEST"+String.valueOf(i)+"_ID";
                               String testTime ="TEST"+String.valueOf(i)+"_TIME";
                               String testID =documentSnapshot.getString(testNo);
                               int testTIME = documentSnapshot.getLong(testTime).intValue();
        ...
        g_firestore.collection("QUIZ").document(g_catList.get(g_selected_cat_index).getDocID())
                ...
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    ...
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        for (DocumentSnapshot document : queryDocumentSnapshots.getDocuments())
                        {
                            String testID =document.getString("TEST_ID");
                            int testTIME = document.getLong("TEST_TIME").intValue();
                            g_testList.add(new TestModel(testID,0,testTIME));
                        }
                        completeListener.onSuccess();
                    }
...
public void onSuccess(DocumentSnapshot documentSnapshot) {
        int noOfTests =g_catList.get(g_selected_cat_index).getNoOfTests();
                       /*for (int i=1;i <= noOfTests; i++)
                       {
                           ...
                           int testTIME = documentSnapshot.getLong("TEST"+String.valueOf(i)+"_TIME").intValue();
                           g_testList.add(new TestModel(testID,0,testTIME));
